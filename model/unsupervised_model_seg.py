...
class Decoder(nn.Module):
    ...
    def forward(self, x, heatmap):
        ...
        x = torch.cat((x[-1], heatmap[0]), dim=1)
        x = self.layer1(x)
...
class Model(nn.Module):
    def __init__(self, n_kps=10, output_dim=200, pretrained=True, output_shape=(64, 64)):
        ...
        config.num_labels = config.num_queries - 1
        # error
        # config.train_size = [32,32]
        ...
        # config.num_latents = 1024

        # config.num_self_attention_heads = 8     
        # config.num_cross_attention_heads = 1

        # print(config.d_model)
        # print(config.d_latents)
        # print(config.cross_attention_shape_for_attention)
        # print(config.cross_attention_widening_factor)        
        # print(config.num_self_attends_per_block)            
        # print(config.num_latents)            
        # print(config.self_attention_widening_factor)                

        # print(config.output_shape)                
        # print(config.num_self_attends_per_block)                
        # print(config.num_self_attention_heads)                
        # print(config.num_frames)

        self.kptNet = DetrForKeypoints(config).from_pretrained("facebook/detr-resnet-50-panoptic",
            config = config,
    ...
    def forward(self, x, tr_x=None, gmtr_x1 = None, gmtr_x2 = None, gmtr_x3 = None):
        ...
        tr_output, tr_x_res, mask_2 = self.kptNet(tr_x)

        if tr_x is not None:
            tr_kpt_out = tr_output.pred_masks

            tr_heatmap = tr_kpt_out.view(-1, self.K, tr_kpt_out.size(2) * tr_kpt_out.size(3))
            tr_heatmap = self.ch_softmax(tr_heatmap)
            tr_heatmap = tr_heatmap.view(-1, self.K, tr_kpt_out.size(2), tr_kpt_out.size(3))
        ...
        kpt_out = output.pred_masks
        heatmap = kpt_out.view(-1, self.K, kpt_out.size(2) * kpt_out.size(3))
        heatmap = self.ch_softmax(heatmap)
        heatmap = heatmap.view(-1, self.K, kpt_out.size(2), kpt_out.size(3))
        ...
        return_dict['tr_kpt_out'] = tr_kpt_out
