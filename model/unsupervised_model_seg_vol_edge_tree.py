...
def sample_tree_from_logits(logits, tau=1.0, hard=False, hard_with_grad=False,
    ...
    if hard_with_grad:
        ...
        hard_samples = hard_samples[:, vertices[0], vertices[1]]
...
class Model(nn.Module):
    ...
    def forward(self, x, tr_x, edge_weights, all_cams, all_cam_items): #gmtr_x1 = None, gmtr_x2 = None, gmtr_x3 = None):
        ...
        for batch_i in range(batch_size):
            ...
            # # rotate
            # coord_volume = coord_volume - base_point.cuda()
            # coord_volume = rotate_coord_volume(coord_volume, theta, axis)
            # coord_volume = coord_volume + base_point.cuda()
    ...
    def _kptToEdge(self, u_x, u_y, v_x, v_y, H=16, W=16, inv_std=1.0/0.1, normalize=False):
        ...
        pairs = np.array(list(itertools.combinations(range(self.K), 2)))
